...
# TODO must improve efficiency greatly, some ideas: don't always go to (p[0], -1), get the bounding box and always go to the nearest point outside the box
def point_inside(p, edges, corners):
    ...
    for p in points_between(p, (p[0], -1)):
        if p in edges:
            for edge in edges[p]:
...
def solve2(input):
    ...
    for i, p1 in enumerate(points):
        ...
        for p2 in points[i + 1 :]:
            area = compute_area(p1, p2)
            if area > maximum:
                valid = valid_pair(p1, p2, edge, corners)
                if valid:
                    maximum = area
